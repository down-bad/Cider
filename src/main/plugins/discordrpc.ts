	...
	private updateActivity(attributes: any) {
		...
		if (!DiscordRichPresence._connection) {
			this._client
				.clearActivity()
				.catch((e: any) =>
					console.error(`[DiscordRichPresence][clearActivity] ${e}`)
				);
			return;
		}
		...
		this._activity = {
			...
			largeImageKey: attributes.artwork.url
				.replace("{w}", "1024")
				.replace("{h}", "1024"),
		...
		if (!attributes.status) {
			if (DiscordRichPresence._store.general.discord_rpc_clear_on_pause) {
				...
				this._client
					...
					.catch((e: any) =>
						console.error(`[DiscordRichPresence][setActivity] ${e}`)
					);
			}
		} else if (
			this._activity &&
			this._activityCache !== this._activity &&
			this._activity.details
		) {
			if (!DiscordRichPresence._store.general.discord_rpc_clear_on_pause) {
			...
			this._client
				.setActivity(this._activity)
				.catch((e: any) =>
					console.error(`[DiscordRichPresence][updateActivity] ${e}`)
				);
			this._activityCache = this._activity;
		}
	}
