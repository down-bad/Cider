...
const app = new Vue({
	...
	methods: {
		songLinkShare(amUrl) {
			...
			httpRequest.open(
				"GET",
				`https://api.song.link/v1-alpha.1/links?url=${amUrl}&userCountry=US`,
				true
			);
			httpRequest.send();
			...
			if (this.lz[message]) {
				if (options["count"]) {
					if (typeof this.lz[message] === "object") {
						let type =
							window.fastPluralRules.getPluralFormNameForCardinalByLocale(
								this.cfg.general.language.replace("_", "-"),
								options["count"]
							);
						return (
							this.lz[message][type] ??
							this.lz[message][Object.keys(this.lz[message])[0]] ??
							this.lz[message]
						);
					} else {
		...
		async showSocialListeningTo() {
			...
			app.showCollection(
				{ data: this.socialBadges.mediaItems },
				"Friends Listening To",
				"albums"
			);
			if (
				this.socialBadges.mediaItemDLState == 1 ||
			...
			await asyncForEach(contentIds, async (item) => {
				try {
					...
					let found = await app.mk.api.v3.music(
						`/v1/catalog/us/${type}/${item}`
					);
		...
		async addSelectedToNewPlaylist() {
			...
			for (let i = 0; i < self.selectedMediaItems.length; i++) {
				if (
					...
					self.selectedMediaItems[i].kind = "albums";
					let res = await self.mk.api.v3.music(
						`/v1/catalog/${app.mk.storefrontId}/albums/${self.selectedMediaItems[i].id}/tracks`
					);
					...
					self.selectedMediaItems[i].kind = "library-albums";
					let res = await self.mk.api.v3.music(
						`/v1/me/library/albums/${self.selectedMediaItems[i].id}/tracks`
					);
					let ids = res.data.data.map(function (i) {
						return { id: i.id, type: i.type };
					});
		...
		async init() {
			...
			let needsReload =
				typeof localStorage["music.ampwebplay.media-user-token"] == "undefined";
			...
			try {
				...
				this.chrome.userinfo = (
					await app.mk.api.v3.music(`/v1/me/social-profile`)
				).data.data[0];
			} catch (err) {}
										...
										if (ids.length > 0) {
											...
											for (id of ids) {
												if (
													!(
														i == 0 &&
														ids[0] == lastItem.attributes.playParams.id
													)
												) {
													try {
			...
			MusicKit.getInstance().videoContainerElement = document.getElementById(
			...
			ipcRenderer.on("play", function (_event, mode, id) {
				if (mode !== "url") {
					self.mk
						.setQueue({ [mode]: id, parameters: { l: self.mklang } })
						.then(() => {
							app.mk.play();
						});
				} else {
			...
			this.mk.addEventListener(MusicKit.Events.nowPlayingItemDidChange, (a) => {
				...
				if (
					...
					document.getElementById("apple-music-video-container").style.display =
						"block";
				...
				if (
					this.cfg.general.playbackNotifications &&
					!document.hasFocus() &&
					a.artistName &&
					a.artwork &&
					a.name
		...
		copyToClipboard(str) {
			if (
				navigator.userAgent.includes("Darwin") ||
				...
				navigator.clipboard
					.writeText(str)
					.then((r) => console.log("Copied to clipboard."));
			}
		},
		...
		async showRecordLabelView(label, title, view) {
			let response = (
				await app.mk.api.v3.music(
					`/v1/catalog/${app.mk.storefrontId}/record-labels/${label}/view/${view}?l=${this.mklang}`
				)
			).data;
		...
		async showSearchView(term, group, title) {
			...
			let response = await app.mk.api.v3.music(
				`/v1/catalog/${app.mk.storefrontId}/search?term=${term}`,
				requestBody,
				{
					includeResponseMeta: !0,
				}
			...
			function getPlaylistTracks(next) {
				app.apiCall(app.musicBaseUrl + next, (res) => {
					...
					self.showingPlaylist.relationships.tracks.data =
						self.showingPlaylist.relationships.tracks.data.concat(res.data);
					if (res.next) {
		...
		async getPlaylistFromID(id, transient = false) {
			...
			const params = {
				...
				"fields[playlists]":
					"curatorName,playlistType,name,artwork,url,playParams",
				"include[library-songs]":
					"catalog,artists,albums,playParams,name,artwork,url",
				"fields[catalog]": "artistUrl,albumUrl,url",
				"fields[songs]":
					"artistUrl,albumUrl,playParams,name,artwork,url,artistName,albumName,durationInMillis",
				l: this.mklang,
			};
			if (!transient) {
			...
			app.mk.api.v3
				...
				.catch((e) => {
					...
					try {
						app.mk.api.v3
							.music(
								`/v1/catalog/${app.mk.storefrontId}/playlists/${id}`,
								params
							)
							.then((res) => {
								self.getPlaylistContinuous(res, transient);
		...
		async getArtistFromID(id) {
			...
			const artistData = await this.mkapi(
				...
				id,
				{
					...
					extend:
						"artistBio,bornOrFormed,editorialArtwork,editorialVideo,isGroup,origin,hero",
		...
		async getSearchHints() {
			...
			let hints = await (
				await app.mk.api.v3.music(
					`/v1/catalog/${app.mk.storefrontId}/search/hints?term=${this.search.term}`
				)
			).data.results;
		...
		routeView(item) {
			...
			if (kind.toString().includes("apple-curator")) {
				...
				let params = {
					...
					views:
						"appears-on,more-by-artist,related-videos,other-versions,you-might-also-like,video-extras,audio-extras",
				};

				if (this.cfg.advanced.experiments.includes("inline-playlists")) {
					let showModal =
						kind.toString().includes("album") ||
						kind.toString().includes("playlist");
				...
				case "artist":
					...
					try {
						if (
							...
							if (
								item.relationships.artists.data[0].type === "artist" ||
								item.relationships.artists.data[0].type === "artists"
							) {
						...
						if (artistId == "") {
							const url =
								item.relationships.catalog.data[0].attributes.artistUrl;
					...
					if (artistId == "") {
						let artistQuery = (
							await app.mk.api.v3.music(
								`v1/catalog/${app.mk.storefrontId}/search?term=${item.attributes.artistName}`,
								{
									limit: 1,
									types: "artists",
								}
				...
				case "album":
					...
					try {
						if ((item.type ?? item.playParams?.kind ?? "") == "albums") {
							...
							if (
								item.relationships.albums.data[0].type === "album" ||
								item.relationships.albums.data[0].type === "albums"
							) {
					...
					if (albumId == "") {
						try {
							let albumQuery = (
								await app.mk.api.v3.music(
									`v1/catalog/${app.mk.storefrontId}/search?term=${
										(item.attributes.albumName ?? item.attributes.name ?? "") +
										" " +
				...
				case "recordLabel":
					...
					if (labelId == "") {
						try {
							let labelQuery = (
								await app.mk.api.v3.music(
									`v1/catalog/${app.mk.storefrontId}/search?term=${item.attributes.recordLabel}`,
									{
										limit: 1,
										types: "record-labels",
									}
		...
		searchLibrarySongs() {
			...
			function sortSongs() {
				...
				self.library.songs.displayListing.sort((a, b) => {
					...
					if (prefs.sort == "dateAdded") {
						let albumida =
							a.relationships?.albums?.data[0]?.id ?? "1970-01-01T00:01:01Z";
						let albumidb =
							b.relationships?.albums?.data[0]?.id ?? "1970-01-01T00:01:01Z";
						aa =
							startTime -
							new Date(
								(albumAdded.find((i) => i[albumida]) ?? [])[albumida] ??
									"1970-01-01T00:01:01Z"
							).getTime();
						bb =
							startTime -
							new Date(
								(albumAdded.find((i) => i[albumidb]) ?? [])[albumidb] ??
									"1970-01-01T00:01:01Z"
							).getTime();
					}
					if (aa == null) {
			...
			if (this.library.songs.search == "") {
				...
				this.library.songs.displayListing = this.library.songs.listing.filter(
					(item) => {
		...
		searchLibraryAlbums(index) {
			...
			if (this.library.albums.search == "") {
				...
				this.library.albums.displayListing = this.library.albums.listing.filter(
					(item) => {
						let itemName = item.attributes.name.toLowerCase();
		...
		async mkapi(
			...
			try {
				if (library) {
					return await this.mk.api.v3.music(
						`v1/me/library/${truemethod}/${term.toString()}`,
						params,
						params2
					);
				} else {
		...
		async getLibrarySongsFull(force = false) {
			...
			this.library.backgroundNotification.message = app.getLz(
				"notification.updatingLibrarySongs"
			);
			...
			function downloadChunk() {
				...
				if (downloaded == null) {
					app.mk.api.v3
						.music(`/v1/me/library/songs/`, params)
						.then((response) => {
						...
						.catch((error) => {
							...
							app.mk.api.v3
								.music(`/v1/me/library/songs/`, safeparams)
								.then((response) => {
									processChunk(response.data);
								})
								.catch((error) => {
			...
			function processChunk(response) {
				...
				self.library.backgroundNotification.message = app.getLz(
					"notification.updatingLibrarySongs"
				);
				...
				if (app.showingPlaylist.relationships.tracks.data.length > 0) {
					...
					return (
						...
						(hours > 0
							? hours +
							  (" " +
									(app.getLz("term.time.hour", (options = { count: hours })) +
										", "))
							: "") +
						(mins > 0
							? mins +
							  (" " +
									app.getLz("term.time.minute", (options = { count: mins })) +
									", ")
							: "") +
						secs +
						(" " +
							app.getLz("term.time.second", (options = { count: secs })) +
							".")
					);
				} else return "";
			} catch (err) {
		...
		async getBrowsePage(attempt = 0) {
			...
			try {
				let browse = await app.mk.api.v3.music(
					`/v1/editorial/${app.mk.storefrontId}/groupings`,
					{
						platform: "web",
						name: "music",
		...
		async getRadioStations(attempt = 0) {
			...
			try {
				this.radio.personal = (
					await app.mk.api.v3.music(`/v1/me/recent/radio-stations`, {
						platform: "web",
						"art[url]": "f",
						l: this.mklang,
					})
		...
		loadAMLyrics() {
			const songID =
				this.mk.nowPlayingItem != null
					? this.mk.nowPlayingItem["_songId"] ??
					  this.mk.nowPlayingItem["songId"] ??
					  -1
					: -1;
			// this.getMXM( trackName, artistName, 'en', duration);
			if (songID != -1) {
				this.mk.api.v3
					.music(`v1/catalog/${this.mk.storefrontId}/songs/${songID}/lyrics`)
					.then((response) => {
		...
		async loadYTLyrics() {
			...
			const time =
				this.mk.nowPlayingItem != null
					? Math.round(
							(this.mk.nowPlayingItem.attributes["durationInMillis"] ?? -1000) /
								1000
					  ) ?? -1
					: -1;
				...
				function loadYT(id, lang) {
					...
					req.onload = function () {
						...
						let captionurl1 = res.substring(
							res.indexOf(`{"playerCaptionsRenderer":{"baseUrl":"`) +
		...
		loadMXM() {
			...
			const track = encodeURIComponent(
				this.mk.nowPlayingItem != null ? this.mk.nowPlayingItem.title ?? "" : ""
			);
						...
						try {
							if (
								...
								lrcfile =
									jsonResponse["message"]["body"]["macro_calls"][
										"track.subtitles.get"
									]["message"]["body"]["subtitle_list"][0]["subtitle"][
										"subtitle_body"
									];
							...
							if (lrcfile == "") {
								...
								if (richsync == [] || richsync.length == 0) {
									...
									for (var i = u.length - 1; i >= 0; i--) {
										...
										let end =
											preLrc.length > 0
												? preLrc[preLrc.length - 1].startTime ?? 99999
												: 99999;
										preLrc.push({
											startTime:
												app.toMS(xline[1].substring(1, xline[1].length - 2)) ??
												0,
											endTime: end,
											line: xline[2],
											translation: "",
						...
						if (status2 == 200) {
							try {
								...
								let translation_list =
									jsonResponse2["message"]["body"]["translations_list"];
								if (translation_list.length > 0) {
									for (var i = 0; i < u.length - 1; i++) {
										...
										for (var trans_line of translation_list) {
											if (
												u[i].line ==
													" " + trans_line["translation"]["matched_line"] ||
												u[i].line == trans_line["translation"]["matched_line"]
											) {
												u[i].translation =
													trans_line["translation"]["description"];
												break;
											}
										}
									}
		...
		parseTTML() {
			...
			let endTimes = [];
			if (
				xml.getElementsByTagName("tt")[0].getAttribute("itunes:timing") ===
				"None"
			) {
				synced = false;
			}
		...
		queueParentandplayChild(parent, childIndex, item) {
			...
			try {
				if (
					app.library.songs.displayListing.length > childIndex &&
					parent == "librarysongs"
				) {
					console.log(item);
					if (
						item &&
						app.library.songs.displayListing[childIndex].id != item.id
					) {
						childIndex = app.library.songs.displayListing.indexOf(item);
					}

					let query = app.library.songs.displayListing.map(
											...
											if (app.showingPlaylist && app.showingPlaylist.id == id) {
												...
												if (app.mk.shuffleMode == 1) {
													...
													for (
														...
														if (
															app.showingPlaylist.relationships.tracks.data[i]
																.id == item.id
														) {
															u.splice(0, i + 1);
															break;
		...
		async searchQuery(term = this.search.term) {
			...
			this.mk.api.v3
				.music(
					`/v1/catalog/${app.mk.storefrontId}/search?term=${encodeURIComponent(
						this.search.term
					)}`,
					{
						types:
				...
				try {
					if (
						...
						if (this.mk["nowPlayingItem"]["attributes"]["artwork"]["url"]) {
							getBase64FromUrl(
								this.mk["nowPlayingItem"]["attributes"]["artwork"]["url"]
									.replace("{w}", size)
									.replace("{h}", size)
							).then((img) => {
				...
				if (
					...
					try {
						document
							...
							.style.setProperty("--artwork", `url("${this.currentArtUrl}")`);
					} catch (e) {}
				} else {
					let data = await this.mk.api.v3.music(
						`/v1/me/library/songs/${this.mk.nowPlayingItem.id}`
					);
					...
					if (
						...
						this.currentArtUrl = (data["attributes"]["artwork"]["url"] ?? "")
							.replace("{w}", 50)
							.replace("{h}", 50);
			...
			try {
				...
				if (data != null && data !== "") {
					document
						...
						.style.setProperty(
							"--artwork",
							'url("' + data["attributes"]["artwork"]["url"].toString() + '")'
						);
				} else {
					document
						.querySelector(".app-playback-controls .artwork")
		...
		async getRating(item) {
			...
			let response = await this.mk.api.v3.music(
				`/v1/me/ratings/${type}?platform=web&ids=${
					type.includes("library") ? item.id : id
				}`
			);
		...
		fetchPlaylist(id, callback) {
			...
			// this.mk.api.
			this.mk.api.v3
				.music(`/v1/catalog/${app.mk.storefrontId}/playlists/${id}`)
				.then((res) => {
				...
				normal: {
					...
					items: [
						...
						},
						{
							...
							action: function () {
								app.mk
									.setStationQueue({ song: app.mk.nowPlayingItem.id })
									.then(() => {
										app.mk.play();
										app.selectedMediaItems = [];
									});
						...
						},
						{
							...
							action: function () {
								app
									.mkapi(
										app.mk.nowPlayingItem.attributes?.playParams?.kind ??
											app.mk.nowPlayingItem.type ??
											"songs",
										false,
										app.mk.nowPlayingItem._songId ??
											app.mk.nowPlayingItem.songId ??
									...
									.then((u) => {
										app.copyToClipboard(
											u.data.data.length && u.data.data.length > 0
												? u.data.data[0].attributes.url
												: u.data.data.attributes.url
										);
									});
							},
						},
		...
		LastFMAuthenticate() {
			...
			window.open(
				"https://www.last.fm/api/auth?api_key=f9986d12aab5a0fe66193c559435ede3&cb=cider://auth/lastfm"
			);
			...
			ipcRenderer.on("LastfmAuthenticated", function (_event, lfmAuthKey) {
				...
				element.innerHTML = `${app.getLz(
					"term.disconnect"
				)}\n<p style="font-size: 8px"><i>(${app.getLz(
					"term.authed"
				)}: ${lfmAuthKey})</i></p>`;
		...
		fullscreen(flag) {
			if (flag) {
				...
				if (
					app.mk.nowPlayingItem.type &&
					app.mk.nowPlayingItem.type.toLowerCase().includes("video")
				) {
					document
						.querySelector("video#apple-music-video-player")
						.requestFullscreen();
				} else {
		...
		isElementOverflowing(selector) {
			try {
				...
				element.setAttribute(
					"data-value",
					"\xa0\xa0\xa0\xa0" + element.textContent
				);
		...
		checkMarquee() {
			if (
				...
				document
					.getElementsByClassName("song-artist")[0]
					.classList.add("marquee");
				document
					.getElementsByClassName("song-artist")[1]
					.classList.add("marquee-after");
			}
			if (
				isElementOverflowing(
				...
				document
					...
					.classList.add("marquee");
				document
					.getElementsByClassName("song-name")[1]
					.classList.add("marquee-after");
			}
		},
		closeWindow() {
		...
		async MKJSLang() {
			...
			try {
				item = await MusicKit.getInstance().api.v3.music(
					`v1/storefronts/${app.mk.storefrontId}`
				);
				...
				if (u && langcodes.includes(u.toLowerCase().replace("_", "-"))) {
					...
					u &&
					u.includes("_") &&
					langcodes.includes(u.toLowerCase().replace("_", "-").split("-")[0])
				) {
					sellang = u.toLowerCase().replace("_", "-").split("-")[0];
				}
				if (sellang == "")
					sellang =
						item.data.data[0].attributes.defaultLanguageTag.toLowerCase();
				console.log(sellang);
				...
				if (sellang.startsWith("en") && this.mk.storefrontId != "us")
					sellang = "en-gb";
				return await sellang;
			}
		},
